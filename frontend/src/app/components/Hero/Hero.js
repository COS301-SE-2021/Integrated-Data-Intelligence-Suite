                ...
                [
                    {
                        angle: 0,
                        s: 0,
                        color: props.s === 0 ? palette[3] : palette[0]
                    },
                    {
                        angle: 0,
                        s: 1,
                        color: props.s === 1 ? palette[3] : palette[1]
                    },
                    {
                        angle: 0,
                        s: 2,
                        color: props.s === 2 ? palette[3] : palette[2]
                    }
                ]
            );
        return (
...
function makeData(nbSeries, nbPoints) {
    return [...Array(nbSeries)
        .keys()].map((d, s) => series(nbPoints, s));
}

function series(nbPoints, s) {
    let previousPoint = random({
        scope: 3,
        rolls: 4
    });
    return [...Array(nbPoints)
        .keys()].map(d => {
        const y =
            previousPoint > 17.5
                ? previousPoint - random({
                scope: 1,
                rolls: 3
            })
                : previousPoint < 2.5
            ...
            >
                ...
                >
                    {this.state.chartsProps
                        ? this.state.chartsProps.slice(0, chartsToRender)
                            .map((d, i) => (
                                <div
                                    key={i}
